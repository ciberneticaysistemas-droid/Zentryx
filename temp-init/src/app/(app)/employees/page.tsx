...
import { UserPlus, Search, Download, X, Edit3, UserX, Check } from 'lucide-react';
...
const STATUS_OPTIONS: [EmployeeStatus, string][] = [
  ['active', 'Activo'], ['probation', 'Período prueba'],
  ['on-leave', 'Licencia'], ['inactive', 'Inactivo'],
];

type EmpForm = Omit<Employee, 'id' | 'initials'>;
...
export default function EmployeesPage() {
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'empleados_zentryx.csv'; a.click();
  ...
  const submitNew = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setForm(blank);
        fetchEmployees();
        toast('success', `${data.employee.name} añadido(a) a la plantilla.`);
      }
  ...
  const openEdit = (emp: Employee) => {
    setEditTarget(emp);
    setForm({ name: emp.name, role: emp.role, department: emp.department, location: emp.location,
      status: emp.status, contractType: emp.contractType, startDate: emp.startDate,
      email: emp.email, phone: emp.phone, salary: emp.salary, manager: emp.manager });
  };
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      setEditTarget(null);
      fetchEmployees();
      toast('success', `Datos de ${form.name} actualizados.`);
    } finally { setSaving(false); }
  };
  ...
  const deactivate = async (emp: Employee) => {
    ...
    fetchEmployees();
    toast('success', `${emp.name} desactivado(a) del sistema.`);
  };

  const field = (key: keyof EmpForm, label: string, type = 'text', placeholder = '') => (
    <div>
  ...
  const sel = (key: keyof EmpForm, label: string, options: [string, string][]) => (
    <div>
      <label className="text-[11px] font-medium block mb-1" style={{ color: 'var(--zx-text-3)' }}>{label}</label>
  ...
  const formBody = (
    <div className="grid grid-cols-2 gap-3">
      ...
      {sel('status', 'Estado', STATUS_OPTIONS)}
  ...
  return (
    <>
      ...
      <div className="flex-1 p-6 space-y-4 animate-fade-in-up">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex-1 min-w-48 flex items-center gap-2 px-3 py-2 rounded-lg text-sm"
            ...
            {search && <button onClick={() => setSearch('')}><X size={12} style={{ color: 'var(--zx-text-3)' }} /></button>}
          </div>
          <Button variant="secondary" size="sm" icon={<Download size={13} />} onClick={exportCSV}>Exportar</Button>
          <Button variant="primary" size="sm" icon={<UserPlus size={13} />} onClick={() => { setForm(blank); setShowNew(true); }}>
            Nuevo empleado
          </Button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="rounded-xl px-4 py-3" style={{ background: 'var(--zx-surface)', border: '1px solid var(--zx-border)' }}>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: 'var(--zx-surface)', border: '1px solid var(--zx-border)' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-8 text-center text-sm" style={{ color: 'var(--zx-text-3)' }}>
                    ...
                    <tr key={emp.id} className="zx-row transition-colors"
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          {emp.status !== 'inactive' && (
                            <button onClick={() => setConfirmDeact(emp)} title="Desactivar"
                              className="p-1.5 rounded-md transition-colors"
                              style={{ color: 'var(--zx-text-3)' }}
                              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--zx-surface-2)'}
                              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
      ...
      <Modal open={showNew} onClose={() => setShowNew(false)} title="Nuevo Empleado" size="xl">
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <Button variant="primary" className="flex-1 justify-center" loading={saving} onClick={submitNew}>
              Agregar empleado
            </Button>
      ...
      {/* Edit Employee Modal */}
      <Modal open={!!editTarget} onClose={() => setEditTarget(null)} title={`Editar — ${editTarget?.name ?? ''}`} size="xl">
        <div className="space-y-3">
          {formBody}
          <div className="flex gap-2 pt-1">
            <Button variant="primary" className="flex-1 justify-center" loading={saving} icon={<Check size={13} />} onClick={submitEdit}>
              Guardar cambios
            </Button>
      ...
      <Modal open={!!confirmDeact} onClose={() => setConfirmDeact(null)} title="Confirmar desactivación" size="sm">
        <div className="space-y-4">
          <p className="text-sm" style={{ color: 'var(--zx-text-2)' }}>
            ¿Desactivar a <strong>{confirmDeact?.name}</strong>? El empleado permanecerá en el sistema
            (historial preservado) pero quedará marcado como inactivo.
          </p>
          <div className="flex gap-2">
            <Button variant="primary" className="flex-1 justify-center"
              onClick={() => confirmDeact && deactivate(confirmDeact)}>
              Sí, desactivar
            </Button>
            <Button variant="secondary" onClick={() => setConfirmDeact(null)}>Cancelar</Button>
          </div>
        </div>
      </Modal>
